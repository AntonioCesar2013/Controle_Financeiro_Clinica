import { renderInstitutionalHeader } from "./institutional-header.js";
...
export function createResidentDocuments({ api, showPanel, showAlert }) {
    async function statementBody(id, start = "", end = "") {
        ...
        const payments = renderActionTable(d.recebimentos, [["Data", "data_recebimento", date], ["Parcela", "numero_parcela"], ["Principal", "valor", money], ["Multa e juros", "multa_juros", money], ["Total recebido", "total_lancamento", money], ["Forma", "forma_recebimento"], ["Estorno", "estornada", v => v ? "Estornado" : "Efetivo"]], row => !row.estornada && row.tipo === "MENSALIDADE" ? `<button class="button" data-action="generate-receipt" data-id="${row.id}">Gerar recibo</button>` : "");
        ...
        return `${controls}<article class="resident-document">${renderInstitutionalHeader()}<header><h2>Extrato por residente</h2><h3>${e(d.residente.nome)}</h3><p>CPF: ${e(formatCpf(d.residente.cpf))}</p><p>Período: ${start ? date(start) : "Início dos registros"} a ${end ? date(end) : "Último registro"} · Emitido em ${date(localDate())}</p></header><div class="metrics">${metric("Recebido no período", r.recebido_periodo, "success")}${metric("Pendente das cobranças do período", r.pendente_periodo, "warning")}${metric("Pendente total de tratamento", r.pendente_total, "warning")}${metric("Saldo atual da carteira", r.carteira_atual, r.carteira_atual < 0 ? "danger" : "primary")}</div><p>O tratamento e a carteira são controles separados. Saldo negativo da carteira é valor a cobrar dos responsáveis.</p><h3>Cobranças — por vencimento</h3><p>Os saldos refletem recebimentos e descontos registrados até agora.</p>${charges}<h3>Recebimentos — por data de pagamento</h3>${payments}<h3>Devoluções do tratamento</h3><p>Total devolvido no período: ${money(r.devolvido_periodo || 0)}</p>${renderTable(d.devolucoes || [], [["Data", "data_devolucao", date], ["Total devolvido", "total_lancamento", money], ["Situação", "estornada", v => v ? "ESTORNADA" : "EFETIVA"], ["Motivo", "motivo"], ["Documento", "documento"], ["Corrigida em", "estornada_em", formatDateTime], ["Motivo da correção", "motivo_estorno"]])}<h3>Carteira — por data da movimentação</h3><p>Saldo de abertura: ${money(r.carteira_abertura)} · Saldo de fechamento: ${money(r.carteira_fechamento)}</p><p>Saldos recalculados conforme os lançamentos válidos atualmente. Movimentos estornados não alteram o saldo.</p>${wallet}</article>`;
    }
    ...
    async function openReceipt(recebimentoId) {
        try {
            ...
            const d = r.dados;
            const body = `<div class="document-controls"><button class="button" data-action="print-document">Imprimir / salvar PDF</button></div><article class="resident-document receipt">${renderInstitutionalHeader()}<header><h2>Recibo de pagamento de mensalidade</h2><strong>${e(r.numero)}</strong></header>${r.cancelado ? "<h2>RECIBO CANCELADO — RECEBIMENTO ESTORNADO</h2>" : ""}${r.total_devolvido ? `<p>Devolução posterior registrada: ${money(r.total_devolvido)}. Este documento preserva o recebimento original.</p>` : ""}<p class="receipt-value">${money(d.total_lancamento)}</p><p>Recebemos o valor de <strong>${money(d.total_lancamento)}</strong>, em <strong>${date(d.data_recebimento)}</strong>, referente à mensalidade de <strong>${e(d.residente_nome)}</strong>, CPF ${e(formatCpf(d.residente_cpf))}.</p><dl><dt>Valor principal</dt><dd>${money(d.valor)}</dd><dt>Multa e juros</dt><dd>${money(d.multa_juros || 0)}</dd><dt>Internação / parcela</dt><dd>${e(d.internacao_id)} / ${e(d.numero_parcela)}</dd><dt>Vencimento da mensalidade</dt><dd>${date(d.data_vencimento)}</dd><dt>Forma de recebimento</dt><dd>${e(d.forma_recebimento)}</dd><dt>Responsável cadastrado</dt><dd>${e(d.responsavel_nome)} · CPF ${e(formatCpf(d.responsavel_cpf))}</dd><dt>Identificação do recebimento</dt><dd>${e(r.recebimento_id)}</dd></dl><p>Este recibo corresponde ao principal acrescido de eventuais multas e juros.</p>${d.valor < d.valor_devido ? `<p>Pagamento parcial em relação ao valor da mensalidade (${money(d.valor_devido)}).</p>` : ""}<p>${e(d.observacao || "")}</p><p>Emitido em ${formatDateTime(r.emitido_em)}</p><div class="receipt-signature">Assinatura do responsável pelo recebimento<br>Clínica da Cruz</div></article>`;
