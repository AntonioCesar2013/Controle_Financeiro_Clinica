...
from src import caixa, configuracoes_financeiras, contas_pagar, contas_receber, despesas, itens
...
class Requisicao(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if rota == "/api/residentes":
            ...
            return self._json({"sucesso": True, "residente": resultado}, HTTPStatus.CREATED)
...
def executar(host="127.0.0.1", porta=8000):
    ...
    print(f"Controle Financeiro disponível em http://{host}:{porta}")
    try:
